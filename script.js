//  favourite list array if it doesn't exists locally
if (localStorage.getItem("favouritesList") == null) {
  localStorage.setItem("favouritesList", JSON.stringify([]));
}

//Fetching Meals form Api and Return a Meal
...
// showing all Meals accorxding to input by user
  ...
  meals.then((data) => { 
    if (data.meals) {
      data.meals.forEach((ele) => { 
        ...
        if (isFav) {  // When meal is favourite  
          html += `
    ...
    } else { // When input doesnt match 404 error is thrown
      html += `
    ...
    document.getElementById("mainsection").innerHTML = html;  // extract mainsection Id from DOM then upadte html
  });
}

//full meal details in mainsection
...
function addRemoveToFavList(id) {
  ...
  if (contain) {  // if value is true then remove from Favourite List otherwise push to array
  ...
  localStorage.setItem("favouritesList", JSON.stringify(arr)); // seting the localStorage when removed or added
  showMealList();
  showFavMealList();
}